# Implementation of INTpress
This is the implementation of "INTpress: Compression-based Quantized In-band Network Telemetry for Low Bandwidth Overhead Monitoring" based on the P4 language.
...
## BMv2 Instructions
This repository provides a simple linear topology (i.e., 3 hops) for a INTpress simple example. 

1. Download the repository to the local.
   
2. Open two terminals for running a simulation.

3. Configure the network and install the INTpress program on BMv2 switches.
   ```
...
5. Insert BMv2 rules.
   ```
   (Terminal 2) python3 [abs_path for the cloned directory]/INTpress/p4src/BMv2/rule/int_controller.py
   ```
...
2. Compile P4 program
   ```
   ./build_tofino.sh [abs_path for the cloned directory]/INTpress/p4src/Tofino/INTpress.p4 INTpress
   ```
   
3. Run switch model
   ```
   $SDE/run_tofino_model.sh -p INTpress
   ```
   
4. Run switch driver
   ```
   $SDE/run_switchd.sh -p INTpress
   bfshell> bfrt_python [abs_path for the cloned directory]/INTpress/p4src/Tofino/rule/bfrt_rule_INTpress.py
   ```
   
5. Packet generation
   ```
   python3 [abs_path for the cloned directory]/INTpress/p4src/Tofino/packets/send_and_receive.py
   ```
